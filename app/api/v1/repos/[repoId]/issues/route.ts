...
import { listRepoIssues } from '@/server/services/repo-resources-service'

type RouteCtx = { params: Promise<Record<string, string>> }

/** GET /api/v1/repos/:repoId/issues — synced issues (DB-backed). */
export const GET = (request: Request, routeCtx: RouteCtx) =>
  withAuth(async ({ request: req }) => {
    ...
    const { items, total } = await listRepoIssues(repoId, query)
