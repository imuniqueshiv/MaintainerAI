...
import { listRepoReleases } from '@/server/services/repo-resources-service'

type RouteCtx = { params: Promise<Record<string, string>> }

/** GET /api/v1/repos/:repoId/releases — synced releases (DB-backed). */
export const GET = (request: Request, routeCtx: RouteCtx) =>
  withAuth(async ({ request: req }) => {
    ...
    const { items, total } = await listRepoReleases(repoId, query)
