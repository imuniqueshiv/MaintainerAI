...
import { listRepoMilestones } from '@/server/services/repo-resources-service'

type RouteCtx = { params: Promise<Record<string, string>> }

/** GET /api/v1/repos/:repoId/milestones — synced milestones (DB-backed). */
export const GET = (request: Request, routeCtx: RouteCtx) =>
  withAuth(async ({ request: req }) => {
    ...
    const { items, total } = await listRepoMilestones(repoId, query)
