...
import { listRepoLabels } from '@/server/services/repo-resources-service'

type RouteCtx = { params: Promise<Record<string, string>> }

/** GET /api/v1/repos/:repoId/labels — synced labels (DB-backed). */
export const GET = (request: Request, routeCtx: RouteCtx) =>
  withAuth(async ({ request: req }) => {
    ...
    const { items, total } = await listRepoLabels(repoId, query)
