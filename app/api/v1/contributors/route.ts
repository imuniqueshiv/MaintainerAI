...
import { listUserContributors } from '@/server/services/repo-resources-service'

/** GET /api/v1/contributors — synced contributors across the user's organizations. */
export const GET = withAuth(async ({ request, user }) => {
  const query = parseSearchParams(request, resourceListQuerySchema)
  const { items, total } = await listUserContributors(user.id, query)
