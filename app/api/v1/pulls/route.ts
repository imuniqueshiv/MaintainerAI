...
import { listUserPulls } from '@/server/services/repo-resources-service'

/** GET /api/v1/pulls — synced pull requests across the user's organizations. */
export const GET = withAuth(async ({ request, user }) => {
  const query = parseSearchParams(request, resourceListQuerySchema)
  const { items, total } = await listUserPulls(user.id, query)
